...
//    only - a typed StitchAiDesignContext there would make every ctx
...
final class StitchAiDesignOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new StitchAiDesignContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new StitchAiDesignSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new StitchAiDesignResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new StitchAiDesignError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new StitchAiDesignResponse($clean['response']);
    ...
    public static function err_from_map($m): ?StitchAiDesignError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new StitchAiDesignError(is_string($code) ? $code : '', $msg);
    }
