...
// StitchAiDesign SDK retry feature
...
class StitchAiDesignRetryFeature extends StitchAiDesignBaseFeature
{
    ...
    public function init(StitchAiDesignContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (StitchAiDesignContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(StitchAiDesignContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(StitchAiDesignContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
