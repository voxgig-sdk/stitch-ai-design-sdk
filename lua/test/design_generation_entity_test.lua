...
describe("DesignGenerationEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:DesignGeneration(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = design_generation_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "design_generation." .. _op, _live and "live" or "unit")
    ...
    -- CREATE
    local design_generation_ref01_ent = client:DesignGeneration(nil)
    local design_generation_ref01_data = helpers.to_map(vs.getprop(
...
function design_generation_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/design_generation/DesignGenerationTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read design_generation test data: " .. entity_data_file)
  end
