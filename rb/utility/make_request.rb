...
module StitchAiDesignUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = StitchAiDesignResponse.new({})
    ...
    if fetch_err
      ...
      response = StitchAiDesignResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = StitchAiDesignResponse.new(fetched)
    else
