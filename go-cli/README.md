# stitch-ai-design-cli

boru-driven command-line client **and** interactive REPL for the StitchAiDesign
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/stitch-ai-design-cli)
make build

# 2. See usage (words, entities, env vars)
./stitch-ai-design-cli --help
...
STITCH_AI_DESIGN_BASE=https://api.example.com ./stitch-ai-design-cli --help

# 6. No arguments -> interactive REPL
./stitch-ai-design-cli
stitch-ai-design> /help
stitch-ai-design> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/stitch-ai-design-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/stitch-ai-design-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `stitch-ai-design>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `design_generation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/stitch-ai-design-cli`. |
...
(`./stitch-ai-design-cli <expr>`) and interactively (the REPL), and expressions compose the
