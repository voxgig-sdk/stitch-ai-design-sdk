...
from stitchaidesign_sdk.core.helpers import get_ctx_prop, to_map
...
class StitchAiDesignContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = StitchAiDesignControl()
        ...
        if isinstance(sp, StitchAiDesignSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, StitchAiDesignResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, StitchAiDesignResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return StitchAiDesignOperation({})
    ...
    def make_error(self, code, msg):
        return StitchAiDesignError(code, msg, self)
