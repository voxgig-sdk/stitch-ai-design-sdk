...
//     (\`new StitchAiDesignSDK(...)\` / \`StitchAiDesignSDK.test(...)\` become
//     \`StitchAiDesignSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'StitchAiDesignSDK'
...
const TEST_SEED = {"entity":{"design_generation":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new StitchAiDesignSDK(...)\` and any
// \`StitchAiDesignSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+StitchAiDesignSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/StitchAiDesignSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
