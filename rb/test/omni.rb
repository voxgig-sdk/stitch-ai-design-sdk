...
module StitchAiDesignOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if StitchAiDesignOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = StitchAiDesignOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| StitchAiDesignOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || StitchAiDesignOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = StitchAiDesignSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = StitchAiDesignError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { StitchAiDesignOmni.errify(err) }
