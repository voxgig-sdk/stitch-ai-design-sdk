...
import (
	...
	omni "github.com/voxgig-sdk/stitch-ai-design-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.StitchAiDesignError); is {
				out := map[string]any{
					"name":    "StitchAiDesignError",
...
func omniCtx(arg any, client *sdk.StitchAiDesignSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.StitchAiDesignSDK); is {
			client = live
