...
// StitchAiDesign SDK pipeline test
//
...
{
    private static function utility(): StitchAiDesignUtility
    {
        return new StitchAiDesignUtility();
    }
    ...
    private static function ctx(array $over = []): StitchAiDesignContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new StitchAiDesignContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new StitchAiDesignOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof StitchAiDesignError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new StitchAiDesignOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = StitchAiDesignMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new StitchAiDesignOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = StitchAiDesignMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new StitchAiDesignOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = StitchAiDesignMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new StitchAiDesignSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = StitchAiDesignMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new StitchAiDesignResponse([]),
            'result' => new StitchAiDesignResult([]),
        ]);
        [, $err] = StitchAiDesignMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new StitchAiDesignSpec([]),
            'response' => null,
            'result' => new StitchAiDesignResult([]),
        ]);
        [, $err] = StitchAiDesignMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new StitchAiDesignSpec(['step' => 's']),
            'response' => new StitchAiDesignResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new StitchAiDesignResult([]),
        ]);
        [, $err] = StitchAiDesignMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new StitchAiDesignSpec(['step' => 's']),
            'response' => new StitchAiDesignResponse(self::resp(200, ['v' => 1])),
            'result' => new StitchAiDesignResult([]),
        ]);
        [, $err] = StitchAiDesignMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new StitchAiDesignSpec(['step' => 's']),
            'response' => new StitchAiDesignResponse(self::resp(200, ['v' => 2])),
            'result' => new StitchAiDesignResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        StitchAiDesignMakeResponse::call($ctx);
    ...
    {
        $preset = new StitchAiDesignResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new StitchAiDesignSpec([]),
            'response' => new StitchAiDesignResponse([]),
            'result' => new StitchAiDesignResult([]),
        ...
        [$got, $err] = StitchAiDesignMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new StitchAiDesignResult([])]);
        [, $err] = StitchAiDesignMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new StitchAiDesignSpec([]), 'result' => null]);
        [, $err] = StitchAiDesignMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new StitchAiDesignSpec(['step' => 's']),
            'result' => new StitchAiDesignResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new StitchAiDesignOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = StitchAiDesignMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new StitchAiDesignSpec(['step' => 's']),
            'result' => new StitchAiDesignResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new StitchAiDesignOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = StitchAiDesignMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new StitchAiDesignResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new StitchAiDesignSpec([]),
            'result' => new StitchAiDesignResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = StitchAiDesignMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = StitchAiDesignMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new StitchAiDesignError('boom', 'boom');
        $utility->fetcher = function (StitchAiDesignContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new StitchAiDesignSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = StitchAiDesignMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (StitchAiDesignContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = StitchAiDesignMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(StitchAiDesignResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        StitchAiDesignMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (StitchAiDesignContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new StitchAiDesignSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = StitchAiDesignMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new StitchAiDesignResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new StitchAiDesignSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = StitchAiDesignMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = StitchAiDesignMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = StitchAiDesignMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new StitchAiDesignResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, StitchAiDesignDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new StitchAiDesignResult(['ok' => false])]);
        $this->expectException(StitchAiDesignError::class);
        StitchAiDesignDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new StitchAiDesignResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', StitchAiDesignMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        StitchAiDesignMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new StitchAiDesignBaseFeature();
        $b = new StitchAiDesignBaseFeature();
        StitchAiDesignFeatureAdd::call($ctx, $a);
        StitchAiDesignFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): StitchAiDesignBaseFeature
    {
        $f = new StitchAiDesignBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        StitchAiDesignFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): StitchAiDesignContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new StitchAiDesignSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = StitchAiDesignPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        StitchAiDesignPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new StitchAiDesignResponse(['headers' => null]),
            'result' => new StitchAiDesignResult([]),
        ]);
        StitchAiDesignResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        StitchAiDesignResultBody::call($ctx);
