# stitch-ai-design-mcp

[MCP](https://modelcontextprotocol.io) server exposing the StitchAiDesign SDK as
two agent tools — `stitch-ai-design_list` and `stitch-ai-design_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/stitch-ai-design-mcp)
make build

# 2. Provide credentials via the environment
export STITCH_AI_DESIGN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user stitch-ai-design \
  -- /absolute/path/to/stitch-ai-design-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./stitch-ai-design-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// stitch-ai-design_list: first page of records
{ "entity": "design_generation" }
{ "entity": "design_generation", "query": { } }

// stitch-ai-design_load: one record by id
{ "entity": "design_generation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/stitch-ai-design-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user stitch-ai-design \
     -- "$PWD"/dist/*/stitch-ai-design-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `stitch-ai-design_list` and `stitch-ai-design_load` tools now appear
   in new sessions. Ask the agent to *"list design_generation using stitch-ai-design"*
   and it calls `stitch-ai-design_list` with `{"entity":"design_generation"}`.
...
```sh
export STITCH_AI_DESIGN_APIKEY=sk_live_xxx            # API key
export STITCH_AI_DESIGN_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `stitch-ai-design_list` tool
...
### Call the `stitch-ai-design_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `stitch-ai-design_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `stitch-ai-design_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `STITCH_AI_DESIGN_APIKEY` | API key sent with every request. |
| `STITCH_AI_DESIGN_BASE` | Optional override of the API base URL. |
...
```sh
./stitch-ai-design-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"stitch-ai-design_load","arguments":{"entity":"design_generation","query":{"id":1}}}}'
```
