...
// StitchAiDesign SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = StitchAiDesignOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(StitchAiDesignOmni::structfix($now, Struct::undef()))
                        !== json_encode(StitchAiDesignOmni::structfix($val, Struct::undef()))) {
