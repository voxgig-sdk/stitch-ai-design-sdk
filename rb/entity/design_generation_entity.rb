...
class DesignGenerationEntity
  ...
  def make
    opts = @_entopts.dup
    DesignGenerationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = StitchAiDesignHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [DesignGeneration, Hash] the current DesignGeneration data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = StitchAiDesignHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of DesignGeneration fields)
  def match_get
  ...
  #
  # @param reqdata [DesignGenerationCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [DesignGeneration, Hash] the created DesignGeneration; raises StitchAiDesignError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = StitchAiDesignHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
