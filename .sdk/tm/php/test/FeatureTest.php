...
// StitchAiDesign SDK feature test
//
...
class FtCtrl extends StitchAiDesignControl
{
...
class FtClient
{
    ...
    private StitchAiDesignUtility $utility;

    public function __construct(StitchAiDesignUtility $utility, array $options)
    {
    ...
    public function get_utility(): StitchAiDesignUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (StitchAiDesignContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public StitchAiDesignContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (StitchAiDesignContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = StitchAiDesignConfig::make_feature($name);
    ...
    private static function buildUrl(StitchAiDesignSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof StitchAiDesignError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new StitchAiDesignResponse($fetched);
        }
    ...
    private function fail(StitchAiDesignContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (StitchAiDesignContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
