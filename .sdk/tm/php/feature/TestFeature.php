...
// StitchAiDesign SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class StitchAiDesignTestFeature extends StitchAiDesignBaseFeature
{
    ...
    public function init(StitchAiDesignContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (StitchAiDesignContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = StitchAiDesignParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (StitchAiDesignContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(StitchAiDesignContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = StitchAiDesignParam::call($ctx, $k);
