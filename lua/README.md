# StitchAiDesign Lua SDK



The Lua SDK for the StitchAiDesign API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:DesignGeneration()` — each with the same small set of operations (`create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
local designgeneration, err = client:DesignGeneration():create({ prompt = "example" })
if err then error(err) end
```
...
| `DesignGeneration` | `(data) -> DesignGenerationEntity` | Create a DesignGeneration entity instance. |
...
| `components` | `table` | Specific UI components to include |
| `design` | `table` |  |
| `designId` | `string` | Unique identifier for the generated design |
| `exportFormats` | `table` | Available export formats for the design |
...
```
lua/
├── stitch-ai-design_sdk.lua    -- Main SDK module
...
The main module (`stitch-ai-design_sdk`) exports the SDK constructor
...
designgeneration:create({ prompt = "example" })

-- designgeneration:data_get() now returns the designgeneration data from the last create
