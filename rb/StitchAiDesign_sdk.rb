...
require_relative 'StitchAiDesign_types'
...
class StitchAiDesignSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = StitchAiDesignHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = StitchAiDesignHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, StitchAiDesignFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    StitchAiDesignUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = StitchAiDesignHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = StitchAiDesignHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = StitchAiDesignHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue StitchAiDesignError => err
    ...
    if fetched.is_a?(Hash)
      status = StitchAiDesignHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = StitchAiDesignError.new(
        "graphql_error", "StitchAiDesignSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = StitchAiDesignSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
