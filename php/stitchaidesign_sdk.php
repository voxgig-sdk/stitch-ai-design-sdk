...
class StitchAiDesignSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = StitchAiDesignHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = StitchAiDesignHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!StitchAiDesignFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, StitchAiDesignFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return StitchAiDesignUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = StitchAiDesignHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = StitchAiDesignHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = StitchAiDesignHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new StitchAiDesignError($op . "_allow",
                "StitchAiDesignSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = StitchAiDesignHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new StitchAiDesignError("graphql_error",
                "StitchAiDesignSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function DesignGeneration($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_design_generation === null) {
                $this->_design_generation = new DesignGenerationEntity($this, null);
            }
            return $this->_design_generation;
        }
        return new DesignGenerationEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new StitchAiDesignSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
