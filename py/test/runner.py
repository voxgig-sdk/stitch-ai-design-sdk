# StitchAiDesign SDK test runner

from __future__ import annotations
import os
import json

from stitchaidesign_sdk.utility.voxgig_struct import voxgig_struct as vs


class StitchAiDesignTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            StitchAiDesignTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = StitchAiDesignTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = StitchAiDesignTestRunner.getenv("STITCH_AI_DESIGN_TEST_LIVE")
        override = StitchAiDesignTestRunner.getenv("STITCH_AI_DESIGN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = StitchAiDesignTestRunner.getenv(key)
        ...
        explain = StitchAiDesignTestRunner.getenv("STITCH_AI_DESIGN_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["STITCH_AI_DESIGN_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if StitchAiDesignTestRunner._test_control is not None:
            return StitchAiDesignTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                StitchAiDesignTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            StitchAiDesignTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = StitchAiDesignTestRunner.load_test_control()
...
def load_env_local():
    StitchAiDesignTestRunner.load_env_local()


def env_override(m):
    return StitchAiDesignTestRunner.env_override(m)


def entity_data(v):
    return StitchAiDesignTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return StitchAiDesignTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return StitchAiDesignTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return StitchAiDesignTestRunner.load_test_control()


def live_client_options():
    return StitchAiDesignTestRunner.live_client_options()


def live_delay_ms():
    return StitchAiDesignTestRunner.live_delay_ms()
